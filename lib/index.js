...
Xmodem.prototype.send = function(socket, dataBuffer) {
  ...
  socket.on('data', function(data) {
    if(data[0] === CRC_MODE && blockNumber === _self.XMODEM_START_BLOCK) {
      ...
      log.info("[SEND] - received NAK byte!");
      ...
      log.warn("GOT SOME OTHER DATA which was not handled properly!");
      log.warn("===>");
      log.warn(data);
      log.warn("<===");
...
function sendBlock(socket, blockNr, blockData, mode) {
  ...
  log.info(sendBuffer);
  if(mode === 'crc') {
    sendBuffer = Buffer.concat([sendBuffer, new Buffer(crc.crc16xmodem(blockData).toString(16), "hex")]);
  }
  else {
